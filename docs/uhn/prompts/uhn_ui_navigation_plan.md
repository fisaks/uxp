...
4. ~~**Technical section search + deep linking**~~ ✅ `5a37fca` — Client-side free-text search with multi-word AND matching on Resources, Views, and Scenes pages. Search term synced to `?search=` URL param with debounced updates. Sticky search bar with focus preservation. Pre-computed search index per page. Route-based deep linking (`/technical/resources/:itemId`) with auto-scroll, persistent highlight ring, and scroll-back chip. Breadcrumb shows item name on deep link pages. Admin-only technical link icon on LocationTile.
5. ~~**Rules delivery + UI**~~ ✅ `1693c06` — Separate rules from runtime overview into dedicated `rule/*` subscription. Rules page in technical section with split-screen layout (desktop: tile grid left, detail panel right; mobile: stacked). Master/edge indicator, triggers visible, trigger resources resolved to names in detail panel. Clear selection button, remove-all resources, per-word search on resource autocomplete.
5b. ~~**Rule action hints**~~ ✅ `d0749f4` — Optional `.actionHints()` on rule builder declaring resources a rule's actions typically affect. Shown in rule detail panel alongside triggers. Bidirectional deep links between rule tiles and resource tiles. "See attached rules" / "Triggered by" / "Action target of" sections in ResourceTile info popover. Documentation in blueprint authoring guide.
6. ~~**Command palette**~~ ✅ `e1a8de4` — Combined search + command palette using MUI Autocomplete (freeSolo, controlled). Inline on home page (sticky command bar), dialog overlay on technical pages (Ctrl+K). Client-side item generation from location-assigned views, resources, and scenes via Redux selectors. Fuzzy search with Damerau-Levenshtein distance (scaled threshold: floor(tokenLength/3)). Actions: navigate, send-command, execute-scene, scroll-to-location, scroll-to-item with highlight ring, analog slider popup for continuous controls, grid filtering. Favorites shown as suggestions on empty query. Keywords support added to blueprint types (resources, views, scenes, locations). Precomputed search text selectors for home page grid filtering. Group names baked into search text at build time.
