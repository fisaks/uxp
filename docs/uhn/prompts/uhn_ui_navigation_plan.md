...
5. **Rules delivery + UI** — Separate rules from runtime overview into dedicated `rule/*` subscription. Rules page in technical section with split-screen layout (desktop: tile grid left, detail panel right; mobile: stacked). Master/edge indicator, triggers visible, trigger resources resolved to names in detail panel.
5b. **Rule action targets** — Optional `.targets()` method on rule builder that declares which resources the rule's `run()` may act on. If not specified, fall back to the rule file's top-level resource imports. Requires blueprint compiler changes to extract and include target metadata in `RuntimeRuleInfo`. Detail panel on rules page shows action targets alongside trigger resources.
