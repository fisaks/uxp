...
export const WebSocketConfig: React.FC<WebSocketConfigProps> = ({ children }) => {
    ...
    const uhnListeners: UHNAppWebSocketResponseListener = useMemo(() => ({
        ...
        "uhn:availability:snapshot": (message) => {
            if (message.payload) dispatch(availabilitySnapshotReceived(message.payload));
        },
        "uhn:availability:change": (message) => {
            if (message.payload) dispatch(availabilityChangeReceived(message.payload.entry));
        },
