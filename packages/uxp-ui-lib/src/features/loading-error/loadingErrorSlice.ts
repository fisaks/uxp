...
export const createLoadingErrorSlice = <T extends string, RootState>(actionNames: readonly T[], loadingError: keyof RootState) => {
    ...
    const createLoadingErrorAwareThunk = <Returned, Payload>
        (actionName: T, asyncFunction: AsyncThunkPayloadCreator<Returned, Payload>

        ) => {
        return createAsyncThunk<Returned, Payload>(
            actionName,
    ...
    const selectActionIsLoading = (action: T) =>
        createSelector(selectLoadingErrorState, (loadingState) => !!loadingState[action].loading);

    const selectActionError = (action: T) =>
        createSelector(selectLoadingErrorState, (loadingState) => !!loadingState[action].error);

    const selectActionIsLoaded = (action: T) =>
        createSelector(selectLoadingErrorState, (loadingState) => !!loadingState[action].loaded);
