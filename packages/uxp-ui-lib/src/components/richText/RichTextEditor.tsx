...
const RichTextEditor: React.FC<RichTextEditorProps> = ({ label, value, onChange, onImageUpload,imageBasePath }) => {
    ...
    return (
        ...
        <Paper ref={editorRootContainerRef}
            elevation={3} sx={{ padding: 0 }} className={`${styles.editorContainer} ${isFullScreen ? styles.fullScreen : ''}`}>
            <Typography variant="caption" color="text.secondary" sx={{ pl: "10px" }}>{label}</Typography>
            <RichEditorToolbar editor={editor} portalContainerRef={portalContainerRef}
                isFullScreen={isFullScreen} toggleFullScreen={toggleFullScreen}
                triggerImageUpload={triggerImageUpload} slotProps={slotProps} />
            <FloatingImageToolbar editor={editor} imagePos={imageToolbarPos} slotProps={slotProps}
