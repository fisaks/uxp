...
export const useRichEditor = ({ editorRootContainerRef, setImageToolbarPos, triggerImageUpload, ...options }: UseEditorOptions & UseEditorMenuOptions, deps?: DependencyList) => {
...
const combinedHandleClick = (
    ...
    timeoutRef: React.MutableRefObject<NodeJS.Timeout | null>) => (view: EditorView,
...
const handleImageClick = (view: EditorView, pos: number, event: MouseEvent,
    setImageToolbarPos: UseEditorMenuOptions["setImageToolbarPos"],
...
const combinedKeyDownHandlers = (triggerImageUpload: () => void) => (view: EditorView, event: KeyboardEvent) => {
    const handlers = [
        (v: EditorView, e: KeyboardEvent) => handleInsertImage(triggerImageUpload, v, e),
    ];
