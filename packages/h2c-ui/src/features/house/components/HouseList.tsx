...
const HouseList: React.FC = () => {
    ...
    const handleUploadFile = (uuid: string, file: File) => {
        return dispatch(uploadFile({ uuid, file })).unwrap().then(r => r.publicId);
    }
            ...
            <List>
                {houses.map((house) => (
                    <Box key={house.uuid} sx={{ borderBottom: `1px solid ${theme.palette.divider}`, mb: 1 }}>
                        <ListItem component={"div"} onClick={() => handleExpandHouse(house.uuid)}
                            sx={{
                                cursor: "pointer",
                            ...
                            <ActionIconButton thunk={deleteHouse} dispatch={dispatch} payload={house.uuid} confirmMessage='Are you sure you want to delete this house?'
                        ...
                        <Collapse in={expandedHouseId === house.uuid} timeout="auto" unmountOnExit>
                            <Box sx={{ pt: 2, pb: 2, backgroundColor: theme.palette.action.selected, borderRadius: '4px' }}>
                                <Box sx={{ backgroundColor: theme.palette.background.paper, pl: 1, pr: 1 }}>
                                    ...
                                    <Box sx={{ mt: 2 }}>
                                        <DebouncedPatchTextField<House> entityId={house.uuid}
                                            label="House Name" field="name" value={house.name} disabled={!editMode}
                                            dispatch={dispatch} patchAction={patchHouseField} />

                                        <DebouncedPatchTextField<House> entityId={house.uuid}
                                            label="Address" field="address" value={house.address} disabled={!editMode}
                                            dispatch={dispatch} patchAction={patchHouseField} />


                                        <DebouncedPatchTextField<House> entityId={house.uuid}
                                            label="Year Built" field="yearBuilt" value={house.yearBuilt} disabled={!editMode}
                                            dispatch={dispatch} patchAction={patchHouseField} />

                                        <DebouncedPatchTextField<House> entityId={house.uuid}
                                            label="Legal Registration Number" field="legalRegistrationNumber" value={house.legalRegistrationNumber} disabled={!editMode}
                                            dispatch={dispatch} patchAction={patchHouseField} />
                                        <RichTextEditor
